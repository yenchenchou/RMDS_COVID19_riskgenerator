...
file_2_clean <- read_csv(paste0('data/weekly_pattern/patterns-',days[2],'.csv'))
file_3_clean <- read_csv(paste0('data/weekly_pattern/patterns-',days[3],'.csv'))
...
risk_final <- risk %>% 
  select(safegraph_place_id, update_date, community, latitude, longitude, risk_score, risk_level) %>% 
